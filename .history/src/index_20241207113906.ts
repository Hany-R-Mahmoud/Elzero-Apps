...
function displayBookmarks() {
  ...
  Object.entries(allBookmarks).forEach((bkmrkCtgry: any, index: number) => {
    bkmrkCtgry[1].forEach((element) => {
      ...
      bkmrkItem.innerHTML = `
                        ...
                        <a class="show-bookmark-title text-decoration-none" target="_blank href="${element.url}"
                          >${element.title}</a
                        >
...
function deleteBookmark(bkmrkCtgry[0], index: number) {
  const allBookmarks = JSON.parse(localStorage.getItem("allBookmarks")) || {};
  console.log(allBookmarks[bkmrkCtgry[0]]);
  console.log(index);
  allBookmarks[bkmrkCtgry[0]].splice(index, 1);
