...
function displayBookmarks() {
  ...
  Object.entries(allBookmarks).forEach((bkmrkCtgry: any, index: number) => {
    // Object.values(bkmrkCtgry[1])

    const bkmrkItem = <HTMLLIElement>document.createElement("li");
    bkmrkItem.className = "bookmark-item";
    console.log(bkmrkCtgry[0]);
    console.log(bkmrkCtgry[1]);
    console.log(bkmrkCtgry[1].forEach(element => {
        return element
    });));
    ...
    //                     <a class="show-bookmark-title text-decoration-none" target="_blank href="${bkmrkCtgry.url}"
    //                       >${bkmrkCtgry.title}</a
    //                     >
