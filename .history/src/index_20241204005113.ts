...
function displayEvents() {
  ...
  allEvents.forEach((newEvent: { name: any; organizer: any; date: any }) => {
    eventsList.innerHTML += `
        <li class="added-event col-lg-4 col-md-6 mb-2">
            <div class="event-box card p-3 border-0 ">
              ...
              <p class="d-flex justify-content-between"><span>Time Left </span>Time</p>
              <button class="delete-btn btn rounded btn-danger"
              onclick="deleteEvent(${index})>Delete</button>
            </div>
          </li>
        `;
  });
...
const eventDeleteBtn = <HTMLButtonElement>(
  document.querySelector(".event-box .delete-btn")
);
const allDeleteBtn = document.querySelectorAll(".event-box .delete-btn");
...
// eventDeleteBtn.addEventListener("click", (index) => {
//   const allEvents = JSON.parse(localStorage.getItem("allEvents"));
//   allEvents.splice(index, 1);
//   localStorage.setItem("allEvents", JSON.stringify(allEvents));
//   displayEvents();
