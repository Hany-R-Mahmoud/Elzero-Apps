...
const bkmrkTitle = (<HTMLInputElement>document.querySelector(".bookmark-title")).value;
const bkmrkUrl = (<HTMLInputElement>document.querySelector(".bookmark-url")).value;
...
for (const bkmrkCtgry in allBookmarks) {
    allBookmarks[bkmrkCtgry].forEach((,index) => {
        
    })
}

    allBookmarks.forEach(
      ( bkmrkCtgry(newBookmark), index: number) => {
        bkmrkList.innerHTML += `
        <li
                  class="bookmark-item all ${bkmrkCtgry}  px-3 py-2 align-items-center justify-content-between rounded mb-2"
                >
