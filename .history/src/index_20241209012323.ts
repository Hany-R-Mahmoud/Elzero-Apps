...
function generatePassword() {
  const pwLength = (document.querySelector(".pw-length") as HTMLInputElement)
    .value;

  pwResult.innerHTML = "";
  if (!numbersCheckbox.checked && !spCharCheckbox.checked) {
    for (let i = 0; i < 10; i++) {
      pwResult.innerHTML += lettersOnlyPool[Math.floor(Math.random() * 10)];
    }
    console.log(pwResult.innerHTML);
  }
}
...
// console.log(...pwPool);
// // console.log(pwPool.match(/[a-z]/gi));
// console.log(pwPool.match(/\D/gi));
// if(checkbox.checked)
