...
function addBookmark() {
  ...
  if (!bkmrkTitle || !bkmrkUrl || !bkmrkCtgry) {
    ...
    //   url: bkmrkUrl,
    // };
    allBookmarks[bkmrkCtgry].push({ bkmrkTitle, bkmrkUrl });
...
function displayBookmarks() {
  ...
  for (const bkmrkCtgry in allBookmarks) {
    allBookmarks[bkmrkCtgry].forEach(
      (newBookmark: { bkmrkUrl: any; bkmrkTitle: any }, index: any) => {
        const bkmrkItem = <HTMLLIElement>document.createElement("li");
        bkmrkItem.classList.add(
          "bookmark-item",
          "all",
        ...
        bkmrkItem.innerHTML = `
                    ...
                    <a class="show-bookmark-title text-decoration-none" target="_blank href="${newBookmark.bkmrkUrl}"
                      >${newBookmark.bkmrkTitle}</a
                    >
                    <button class="delete-bookmark-btn btn btn-danger rounded"
...
function deleteBookmark(bkmrkCtgry: string | number, index: number) {
  ...
  //   Object.entries(allBookmarks).forEach((category: any, index: number) => {
