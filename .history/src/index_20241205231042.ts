...
const bkmrkUrl = (document.querySelector(".bookmark-url") as HTMLInputElement)
  .value;
const bkmrkCtgry = (
  document.querySelector(".bookmark-category") as HTMLInputElement
).value;
...
function addBookmark() {
  if (!bkmrkTitle.value || !bkmrkUrl.value || !bkmrkCtgry.value) {
    Swal.fire({
      text: "Please Fill All Fields",
      icon: "question",
    });
  } else {
    if (!allBookmarks[bkmrkCtgry.value]) allBookmarks[bkmrkCtgry.value] = [];
  ...
  (bkmrkList.innerHTML = ""),
    allBookmarks.forEach(
      (
        ...
        bkmrkList.innerHTML += `
        <li
                  class="bookmark-item all ${newBookmark.category}  px-3 py-2 align-items-center justify-content-between rounded mb-2"
                >
