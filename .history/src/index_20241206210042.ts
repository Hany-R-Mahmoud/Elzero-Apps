...
function displayBookmarks() {
  ...
  Object.entries(allBookmarks).forEach((bkmrkCtgry: any) => {
    bkmrkCtgry[1].forEach((newBookmark, index: number) => {
      const bkmrkItem = <HTMLLIElement>document.createElement("li");
      bkmrkItem.className = "bookmark-item";
      ...
      bkmrkItem.innerHTML = `
                  <span class="show-category-name rounded">${bkmrkCtgry[0]}</span>
                  ...
                  onclick="deleteBookmark(${bkmrkCtgry[0]} ,${index})">
                    Delete
                  </button>
                `;
      ...
      //   ctgryTabs.innerHTML += `
      //             <li class="chosen-category-tab active btn rounded" data-cat=".${bkmrkCtgry[0]}">
      //             ${bkmrkCtgry[0]}
      //                       </li>
      //             `;
    });
  ...
  //     );
  //   }
  //   Object.entries(allBookmarks).forEach((obj) => {
  //     obj[1].forEach((book) => {
  //       console.log(book.title);
}
displayBookmarks();

function deleteBookmark(bkmrkCtgry, index: number) {
  const allBookmarks = JSON.parse(localStorage.getItem("allBookmarks"));

  allBookmarks[bkmrkCtgry].splice(index, 1);
  console.log(allBookmarks);
  console.log(allBookmarks[bkmrkCtgry]);
  console.log(Object.entries(allBookmarks));
...
document.querySelectorAll(".navbar .nav-item a").forEach((item) => {
