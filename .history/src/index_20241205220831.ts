...
const bkmrkTitle = <HTMLInputElement>document.querySelector(".bookmark-title");
const bkmrkUrl = <HTMLInputElement>document.querySelector(".bookmark-url");
...
const bkmrkItem = <HTMLLIElement>document.querySelector(".bookmark-item");
...
const allChosenCtgryTabs = <NodeListOf<HTMLLIElement>>(
  document.querySelectorAll(".category-tabs li")
);
const allBkmrkItems = <NodeListOf<HTMLLIElement>>(
  document.querySelectorAll(".bookmark-list li")
);

function addBookmark() {
  const newBookmark = {
    title: bkmrkTitle.value,
    url: bkmrkUrl.value,
    category: bkmrkCtgry.value,
    choosenCategory: chosenCtgryName.innerHTML,
  };
  allBookmarks.push(newBookmark);
  localStorage.setItem("allBookmark", JSON.stringify(allBookmarks));
  ...
  (bkmrkList.innerHTML = ""),
    allBookmarks.forEach(
      (
        ...
        bkmrkList.innerHTML += `
        <li
                  class="bookmark-item all ${newBookmark.category} d-flex px-3 py-2 align-items-center justify-content-between rounded mb-2"
                >
                  <span class="show-category-name rounded">${newBookmark.category}</span>
                  <a class="show-bookmark-title text-decoration-none" href="${newBookmark.url}"
                    >${newBookmark.title}</a
                  >
        ...
        ctgryTabs.innerHTML += `
        <li class="chosen-category-tab active btn rounded" data-cat=".${newBookmark.category}">
